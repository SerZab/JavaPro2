package de.telran.practice.lesson26_01082023;
...
//        System.out.println(stringToDateTime(datetime).getDayOfWeek());
    ...
    public static long workingDaysBetween(LocalDate start, LocalDate end){
        long workDays = 0; // счетчик раб.дней
        // перебираем дни между start и end (включительно)
        while (!start.isAfter(end)) {
            // проверяем, является ли текущий день рабочим днем (не суббота и не воскресенье)
